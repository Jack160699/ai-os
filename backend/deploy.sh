...
# Run from backend root after git clone, e.g. /opt/ai-os/backend
#
# Usage:
#   chmod +x deploy.sh
#   ./deploy.sh
...
if [[ ! -f "run.py" ]] || [[ ! -f "requirements.txt" ]]; then
  echo "Error: run deploy.sh from the backend directory (where run.py lives)." >&2
  exit 1
fi
...
echo "  2. If not using /opt/ai-os/backend, edit WorkingDirectory and ExecStart in ai-os.service to match ${ROOT}"
