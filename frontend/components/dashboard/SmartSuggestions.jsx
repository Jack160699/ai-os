    ...
    <div className="space-y-3">
      {[0, 1, 2].map((k) => (
        <div key={k} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-3.5">
          <div className="admin-skeleton h-3.5 w-1/2" />
          <div className="admin-skeleton mt-2 h-3 w-11/12" />
        </div>
      ))}
    </div>
  );
...
export function SmartSuggestions({ items = [], loading = false }) {
  return (
    <SurfaceCard className="p-6" delay={0.06}>
      <p className="text-sm font-semibold tracking-tight text-white">Smart Suggestions</p>
      <p className="mt-1 text-[12px] text-slate-500">Actionable AI cues. Tap any suggestion to jump to execution.</p>
        ...
        {loading ? (
          ...
          items.slice(0, 5).map((item) => (
            <Link
              key={item.id}
              href={item.href}
              className="group rounded-xl border border-white/[0.06] bg-gradient-to-br from-white/[0.04] to-white/[0.015] p-3.5 transition-[transform,border-color,box-shadow,background-color] duration-200 hover:-translate-y-0.5 hover:border-sky-400/35 hover:bg-white/[0.05] hover:shadow-[0_14px_42px_rgba(14,165,233,0.18)]"
            >
              <div className="flex gap-3">
                <div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border border-sky-400/30 bg-sky-400/10 transition-colors duration-200 group-hover:border-sky-400/45 group-hover:bg-sky-400/15">
