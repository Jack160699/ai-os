/**
 * Razorpay checkout API — works when Next `/api/*` is missing in production (static / wrong root).
 * Tries same-origin proxy first, then Flask bot URL (needs CORS on Flask).
 *
 * Env (optional):
 * - NEXT_PUBLIC_BOT_API_URL — e.g. https://bot.stratxcel.ai
 * - NEXT_PUBLIC_CHECKOUT_API_BASE — overrides bot base for checkout only
 */
...
export async function checkoutFetch(path, body) {
  ...
  for (const { url } of attempts) {
    ...
    // Missing Next route (404) or SPA fallback (200 HTML for unknown /api/*)
