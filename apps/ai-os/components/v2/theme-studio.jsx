...
export function ThemeStudio({ variant = "default" }) {
  ...
  return (
    <div className="relative">
      <div
        className={
          minimal
            ? "rounded-lg border border-[var(--v2-border)] bg-[var(--v2-elevated)]/80 px-2.5 py-2"
            : "rounded-xl border border-[var(--v2-border)] bg-[var(--v2-elevated)] p-3"
        }
      >
        {minimal ? (
          <div className="flex items-center justify-between gap-2">
            <span className="min-w-0 truncate text-[11px] font-medium tracking-[-0.01em] text-[var(--v2-text)]">{theme.name}</span>
            <button
              type="button"
              onClick={() => setOpen((prev) => !prev)}
              className="shrink-0 rounded-md border border-[var(--v2-border)] bg-[var(--v2-panel)] px-2 py-1 text-[10px] font-medium text-[var(--v2-muted)] transition hover:border-[var(--v2-focus)] hover:text-[var(--v2-text)] active:scale-[0.98]"
            >
              Theme
            </button>
          </div>
      ...
      {toast ? (
        <p className={`text-[11px] text-[var(--v2-muted)] ${minimal ? "mt-1.5" : "mt-2"}`}>{toast}</p>
      ) : null}
