  ...
  try {
    ...
    if (missing.length > 0) {
      return Response.json(
        {
          ok: false,
          error: `LIVE KEY MISSING: ${missing.join(", ")}`,
          reason:
            "These variables are undefined or blank inside this Vercel serverless function. " +
            "Add them to this Vercel project → Settings → Environment Variables for the same " +
            "environment as VERCEL_ENV (Production/Preview), then redeploy. " +
            "They must not be client-only (do not rely on NEXT_PUBLIC_* for secrets).",
          diagnostic: {
            key_id_trimmed_length: liveKeyId.length,
            secret_trimmed_length: liveKeySecret.length,
